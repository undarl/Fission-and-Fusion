...
if settings['startup']['undarl-enable-fission'].value then
	table.insert(data.raw.technology['nuclear-fuel-reprocessing'].effects, {type = "unlock-recipe", recipe = "undarl-reprocess-portable-uranium-cell"})
end
...
if settings['startup']['undarl-enable-fusion'].value then
	...
	if settings['startup']['undarl-enable-fission'].value then
		table.insert(data.raw.technology["fusion-reactor-equipment"].prerequisites, "undarl-portable-reactors")
	end
...
if settings['startup']['undarl-enable-radioisotopes'].value then
	table.insert(data.raw.recipe['nuclear-fuel-reprocessing'].results, {name = "undarl-radioisotopes", probability = 0.50, amount = 1})
	--Add radioisotopes to portable nuclear fuel processing
	if settings['startup']['undarl-enable-fission'].value then
		table.insert(data.raw.recipe['undarl-reprocess-portable-uranium-cell'].results, {name = "undarl-radioisotopes", probability = 0.50, amount = 1})
	end
end
